export class InvoiceController {
  ...
  async load() {
    ...
    try {
      const [invoices, customers, services, appointments] = await Promise.all([
        this.repository.getAll(),
        this.customerRepository.getAll(),
        this.serviceRepository.getAll(),
        new (await import('../appointments/repositories/MockAppointmentRepository.js')).MockAppointmentRepository().getAll(),
      ]);
      this.setState({ invoices, customers, services, appointments, loading: false });
    } catch (error) {
  ...
  resolveInvoice(invoice) {
    ...
    return {
      ...invoice,
      customer: this.state.customers.find((c) => c.id === invoice.customerId),
      service: this.state.services.find((s) => s.id === invoice.serviceId),
      appointment: this.state.appointments.find((a) => a.id === invoice.appointmentId),
    };
  }

  getEnrichedInvoices() {
    return this.state.invoices.map((invoice) => this.resolveInvoice(invoice));
  }
